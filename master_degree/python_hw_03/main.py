# Важно!!!
# list_products возвращает строку, а не печатает ее сама
# поэтому в тестовом кейсе надо обернуть вызов в print(store.list_products())

from entities.product import Product
from entities.store import Store
from tests.unittests import TestStoreOrderProduct


def main():
    # Создаем магазин
    store = Store()

    # Создаем товары
    product1 = Product("Ноутбук", 1000, 5)
    product2 = Product("Смартфон", 500, 10)

    # Добавляем товары в магазин
    store.add_product(product1)
    store.add_product(product2)

    # Список всех товаров
    print(store.list_products())

    # Создаем заказ
    order = store.create_order()

    # Добавляем товары в заказ
    order.add_product(product1, 2)
    order.add_product(product2, 3)

    # Выводим общую стоимость заказа
    total = order.calculate_total()
    print(f"Общая стоимость заказа: {total}")

    # Проверяем остатки на складе после заказа
    print(store.list_products())

    # unit-тесты
    TestStoreOrderProduct()


if __name__ == "__main__":
    main()
