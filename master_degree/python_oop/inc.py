# Приватные поля - доступ либо через методы, либо через костыли
class Wallet:
...
# print(gleb_walley.__name) # Ошибка -> нет такого поля (private)
print(gleb_walley._Wallet__name) 
gleb_walley.show_my_money()


# Протектед поля - доступ напрямую
class UnsafeWallet:
...
print(gleb_unsafe_walley._name) # Ошибки нет
